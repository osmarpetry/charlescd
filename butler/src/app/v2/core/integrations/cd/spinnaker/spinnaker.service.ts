...
import { ICdServiceStrategy, IConnectorConfiguration } from '../../integrations/cd/interfaces'
...
export class SpinnakerService implements ICdServiceStrategy {
  ...
  private getSpinnakerConfiguration(configuration: IConnectorConfiguration): ISpinnakerPipelineConfiguration {
    ...
    return {
      ...
      applicationName: `${DeploymentsConfigurationConstants.SPINNAKER_APPLICATION_PREFIX}${configuration.applicationName}`,
  ...
  private getDeployRetryCondition(deployError: Observable<any>) {
    ...
    return deployError.pipe(
      concatMap((error, attempts) => {
        return attempts >= ConfigurationConstants.CD_CONNECTION_MAXIMUM_RETRY_ATTEMPTS ?
  ...
  private getDeployRetryPipe(error: any, attempts: number) {
    ...
    return of(error).pipe(
      ...
      delay(ConfigurationConstants.CD_CONNECTION_MILLISECONDS_RETRY_DELAY)
    )
  }
